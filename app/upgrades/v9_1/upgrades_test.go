...
import (
	...
	v9 "github.com/AyrisDev/vinceChain/v12/app/upgrades/v9_1"
	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				...
				coins := sdk.NewCoins(sdk.NewCoin("avce", res))
				...
				suite.Require().Equal(balanceBefore.AmountOf("avce"), sdk.NewDecFromInt(res))
			},
			true,
		},
		{ //nolint:dupl
		...
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.expectedSuccess {
				for i := range v9.Accounts {
					...
					balance := suite.app.BankKeeper.GetBalance(suite.ctx, addr, "avce")
