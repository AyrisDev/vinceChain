...
func DeployContract(
	...
	chainID := vinceApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	nonce := vinceApp.EvmKeeper.GetNonce(ctx, from)
	...
	msgEthereumTx := evm.NewTx(&evm.EvmTxArgs{
		...
		Nonce:     nonce,
		GasLimit:  gas,
		GasFeeCap: vinceApp.FeeMarketKeeper.GetBaseFee(ctx),
	...
	res, err := DeliverEthTx(vinceApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, vinceApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := vinceApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := vinceApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := vinceApp.EvmKeeper.GetNonce(ctx, from)
