...
func PrepareEIP712CosmosTx(
	...
	accNumber := appvince.AccountKeeper.GetAccount(ctx, from).GetAccountNumber()

	nonce, err := appvince.AccountKeeper.GetSequence(ctx, from)
	if err != nil {
		return nil, err
	}
...
func createTypedData(args typedDataArgs, useLegacy bool) (apitypes.TypedData, error) {
	if useLegacy {
		...
		vinceCodec := codec.NewProtoCodec(registry)
