...
import (
	...
	"github.com/AyrisDev/vinceChain/v12/testutil/tx"
)
...
//  3. EndBlock
//  4. Commit
func Commit(ctx sdk.Context, app *app.Vince, t time.Duration, vs *tmtypes.ValidatorSet) (sdk.Context, error) {
...
func DeliverTx(
	...
	return BroadcastTxBytes(appvince, txConfig.TxEncoder(), tx)
}
...
func DeliverEthTx(
	...
	tx, err := tx.PrepareEthTx(txConfig, appvince, priv, msgs...)
	if err != nil {
		return abci.ResponseDeliverTx{}, err
	}
	return BroadcastTxBytes(appvince, txConfig.TxEncoder(), tx)
...
func CheckTx(
	...
	return checkTxBytes(appvince, txConfig.TxEncoder(), tx)
}
...
func BroadcastTxBytes(app *app.Vince, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseDeliverTx, error) {
...
func checkTxBytes(app *app.Vince, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseCheckTx, error) {
	bz, err := txEncoder(tx)
	if err != nil {
		return abci.ResponseCheckTx{}, err
	}
