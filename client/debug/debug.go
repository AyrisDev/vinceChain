// Copyright 2022 vince Foundation
...
func LegacyEIP712Cmd() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s debug legacy-eip712 tx.json --chain-id vinced_1903-1`, version.AppName),
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			chainID, err := vince.ParseChainID(clientCtx.ChainID)
			if err != nil {
