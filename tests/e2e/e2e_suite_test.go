...
// runInitialNode builds a docker image capable of running an vince node with the given version.
...
func (s *IntegrationTestSuite) runInitialNode(version upgrade.VersionConfig) {
	...
	s.Require().NoError(err, "can't build container with vince version: %s", version.ImageTag)
	...
	s.Require().NoError(err, "can't run node with vince version: %s", version)
...
// runNodeWithCurrentChanges builds a docker image using the current branch of the vince repository.
...
func (s *IntegrationTestSuite) runNodeWithCurrentChanges() {
	...
	err = s.upgradeManager.RunNode(node)
	s.Require().NoError(err, "can't run node vince using branch %s", branch)
}
...
func (s *IntegrationTestSuite) proposeUpgrade(name, target string) {
	...
	// if vince is lower than v10.x.x no need to use the legacy proposal
	currentVersion, err := s.upgradeManager.GetNodeVersion(ctx)
	s.Require().NoError(err, "can't get current vince version")
	...
	s.Require().NoErrorf(
		err,
		"can't create the proposal to upgrade vince to %s at height %d with name %s",
	...
	s.Require().NoErrorf(
		err,
		"failed to submit proposal to upgrade vince to %s at height %d\nstdout: %s,\nstderr: %s",
...
func (s *IntegrationTestSuite) voteForProposal(id int) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	exec, err := s.upgradeManager.CreateVoteProposalExec(s.upgradeParams.ChainID, id, "--fees=500avce", "--gas=500000")
...
func (s *IntegrationTestSuite) upgrade(targetRepo, targetVersion string) {
	...
	// export node .vinced to local build/
	...
	node.SetCmd([]string{"vinced", "start", fmt.Sprintf("--chain-id=%s", s.upgradeParams.ChainID)})
	err = s.upgradeManager.RunNode(node)
