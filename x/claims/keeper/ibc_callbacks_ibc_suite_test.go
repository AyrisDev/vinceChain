...
func (suite *IBCTestingSuite) SetupTest() {
	...
	err = testutil.FundModuleAccount(suite.chainB.GetContext(), suite.chainB.App.(*app.Vince).BankKeeper, types.ModuleName, coins)
	suite.Require().NoError(err)

	suite.chainB.App.(*app.Vince).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), addr, claimsRecord)

	err = testutil.FundModuleAccount(suite.chainA.GetContext(), suite.chainA.App.(*app.Vince).BankKeeper, types.ModuleName, coins)
	suite.Require().NoError(err)
	...
	err = suite.chainA.App.(*app.Vince).ClaimsKeeper.SetParams(suite.chainA.GetContext(), params)
	suite.Require().NoError(err)
	err = suite.chainB.App.(*app.Vince).ClaimsKeeper.SetParams(suite.chainB.GetContext(), params)
	suite.Require().NoError(err)
...
func (suite *IBCTestingSuite) TestOnAcknowledgementPacketIBC() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			func(_ int64) {
				...
				suite.chainA.App.(*app.Vince).ClaimsKeeper.SetParams(suite.chainA.GetContext(), params) //nolint:errcheck
				suite.chainB.App.(*app.Vince).ClaimsKeeper.SetParams(suite.chainB.GetContext(), params) //nolint:errcheck
			},
			4,
			0,
			false,
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.Require().Equal(sdk.NewCoin("avce", sdk.NewInt(tc.expectedBalance)).String(), coin.String())
...
func (suite *IBCTestingSuite) TestOnRecvPacketIBC() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("avce", sdk.NewInt(claimableAmount/4)))

				suite.chainB.App.(*app.Vince).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), senderAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{false, true, true, true}})
				// update the escrowed account balance to maintain the invariant
				err := testutil.FundModuleAccount(suite.chainB.GetContext(), suite.chainB.App.(*app.Vince).BankKeeper, types.ModuleName, coins)
				suite.Require().NoError(err)
			},
			func() {
				...
				_, found := suite.chainB.App.(*app.Vince).ClaimsKeeper.GetClaimsRecord(suite.chainB.GetContext(), senderAddr)
				suite.Require().True(found)
			},
			4,
			0,
		...
		},
		{
			...
			func(claimableAmount int64) {
				...
				suite.chainB.App.(*app.Vince).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), senderAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{false, false, false, true}})
				suite.chainB.App.(*app.Vince).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), receiverAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{true, true, true, false}})
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("avce", amt.QuoRaw(2)))

				suite.chainB.App.(*app.Vince).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), senderAddr, types.ClaimsRecord{InitialClaimableAmount: sdk.ZeroInt(), ActionsCompleted: []bool{false, false, false, false}})
				suite.chainB.App.(*app.Vince).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), receiverAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{false, true, true, false}})
