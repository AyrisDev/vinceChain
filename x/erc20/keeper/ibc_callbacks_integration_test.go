...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert avce", func() {
			...
			avceInitialBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)

			// 1. Send avce from Evmos to Osmosis
			s.SendAndReceiveMessage(s.pathOsmosisEvmos, s.EvmosChain, utils.BaseDenom, amount, receiver, sender, 1, "")

			avceAfterBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)
			...
			// 2. Send avce IBC coins from Osmosis to Evmos
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.Avincedenomtrace.Path, teststypes.Avincedenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisEvmos, s.IBCOsmosisChain, teststypes.avceIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)
			...
			avceFinalBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)
			...
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, teststypes.avceIbcdenom)
	...
	Describe("Performing claims with registered coin", func() {
		...
		It("it should perform the claim and convert the received tokens", func() {
			...
			receiverInitialavceBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)
			...
			receiverFinalavceBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)
